...
const { Authenticate, ensureFranchise } = require("../services/middleware/authenticate")
...
router.get("/", ensureFranchise, (req, res) => {
  res.send("Hello Franchise, this is 'home' page ")
})
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (userExist && userExist.role === roles.franchise) {
      ...
      if (isMatch) {
        res.send({ message: `${userExist.name} (FRANCHISE) Logged In Sucessfully !`, token })
      } else {
...
router.post("/logout", Authenticate, async (req, res) => {
  try {
    ...
    res.status(200).send(`${req.userData.name} (FRANCHISE) Logged out !`)
  } catch (error) {
    res.status(500).send(error)
  }
})

router.get("/dashboard", Authenticate, ensureFranchise, (req, res) => {
...
router.post("/dashboard/update", Authenticate, ensureFranchise, async (req, res) => {
  try {
...
router.get("/dashboard/students", Authenticate, ensureFranchise, async (req, res) => {
  try {
    const cityArr = req.userData.city.toString().split("-")
    let studentsData = await User.find({ role: roles.user, city: { $in: [...cityArr] } })
...
router.post("/dashboard/students/filter", Authenticate, ensureFranchise, async (req, res) => {
  try {
    let studentsData = await User.find({ role: roles.user, ...req.body })
    if (studentsData.length === 0) return res.status(204).send()
    studentsData = studentsData.map((userData) => {
      const { tests, _id, name, gender, Class, email, school, mobile, state, city, role, img } = userData
      return { tests, _id, name, gender, Class, email, school, mobile, state, city, role, img }
...
router.post("/dashboard/students/search", Authenticate, async (req, res) => {
  const cityArr = req.userData.city.toString().split("-")
  let queryObj = { role: roles.user }
  if (req.body.city) queryObj.city = req.body.city
  if (req.body.Class) queryObj.Class = req.body.Class
  if (req.body.state) queryObj.state = req.body.state
  if (req.body.school) queryObj.school = req.body.school
  let users = []
  ...
  users = users.filter((user) => cityArr.includes(user.city))
  res.json(users)
})

router.post("/dashboard/students", Authenticate, ensureFranchise, async (req, res) => {
  try {
...
router.post("/dashboard/students/update", Authenticate, ensureFranchise, async (req, res) => {
  try {
...
router.delete("/dashboard/students", Authenticate, ensureFranchise, async (req, res) => {
  try {
    const { _id } = req.body
