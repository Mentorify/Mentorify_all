...
const { Authenticate, ensureAdmin } = require("../services/middleware/authenticate")
const randomString = require("randomstring")
const addUserMail = require("../services/sendMail/addUserMail")
const sendMail = require("../services/sendMail/addUserMail")
...
router.get("/", ensureAdmin, (req, res) => {
  res.send("Hello Admin, this is 'home' page ")
})
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (userExist && userExist.role === roles.admin) {
      ...
      if (isMatch) {
        res.send({ message: `${userExist.name} (ADMIN) Logged In Sucessfully !`, token })
      } else {
...
router.post("/logout", Authenticate, async (req, res) => {
  try {
    ...
    res.status(200).send(`${req.userData.name} (ADMIN) Logged out !`)
  } catch (error) {
    res.status(500).send(error)
  }
})

router.get("/dashboard", Authenticate, ensureAdmin, (req, res) => {
...
router.post("/dashboard/update", Authenticate, ensureAdmin, async (req, res) => {
  try {
...
router.get("/dashboard/students", Authenticate, ensureAdmin, async (req, res) => {
  try {
...
router.post("/dashboard/students/filter", Authenticate, ensureAdmin, async (req, res) => {
  try {
...
router.post("/dashboard/students/search", async (req, res) => {
  const users = await User.find({ email: { $regex: req.body.searchKey, $options: "i" } })
  res.send(users)
})

router.post("/dashboard/students", Authenticate, ensureAdmin, async (req, res) => {
  try {
...
router.post("/dashboard/students/update", Authenticate, ensureAdmin, async (req, res) => {
  try {
...
router.delete("/dashboard/students", Authenticate, ensureAdmin, async (req, res) => {
  try {
    const { _id } = req.body
